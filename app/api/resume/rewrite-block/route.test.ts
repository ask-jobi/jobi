...
const mockRewriteBlock = rewriteBlock as jest.MockedFunction<typeof rewriteBlock>;
...
describe('POST /api/resume/rewrite-block', () => {
  ...
  describe('Server error scenarios', () => {
    ...
    it('should return 500 error when request body parsing fails', async () => {
      const invalidRequest = new Request('http://localhost:3000/api/resume/rewrite-block', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  describe('Edge cases', () => {
    it('should handle content with special characters', async () => {
      const specialCharBody: RewriteBlockRequest = {
        ...validRequestBody,
        originalContent: '负责前端开发工作，使用React & TypeScript，处理过100+个bug',
        instruction: '突出技术栈 & 量化成果',
      };

      const mockResponse = {
        optimizedContent: '使用React和TypeScript进行前端开发，解决了100多个技术问题',
