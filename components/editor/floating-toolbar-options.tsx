...
function FloatingToolbarOptions({ editor }: { editor: Editor }) {
  ...
  const intoAiMode = () => {
    editor.chain().expandSelectionToNodeEdge().setMode("ai").run()
  }

  return (
    <div className="flex items-center px-2 py-1.5 rounded-lg shadow-xl border bg-white pointer-events-auto">
      <Button type="button" variant="outline" size="icon" onClick={intoAiMode}>
      ...
      >
        ...
        <SelectContent
          ...
          className="shadow-xl z-102"
        >
          <SelectItem value="paragraph" icon={ALargeSmallIcon}>
            Normal
          </SelectItem>
          <SelectSeparator />
          <SelectItem value="bullet" icon={ListIcon}>
            Bullet List
          </SelectItem>
          <SelectItem value="number" icon={ListOrderedIcon}>
            Ordered List
          </SelectItem>
        </SelectContent>
      </Select>
