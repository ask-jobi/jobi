...
function ReasoningText({
  ...
  const getTarget = isStreaming ? () => contentRef.current as HTMLElement : null
  ...
  return (
    <div
      ...
      className={cn(
        "aui-reasoning-text relative z-0 max-h-32 overflow-y-auto pt-2 pb-2 pl-6 leading-relaxed",
...
const ReasoningGroupImpl: ReasoningGroupComponent = ({
  ...
  const isReasoningComplete = useAuiState((s) => {
    const reasoningParts = s.message.parts.filter(
      (p, i): p is typeof p & { status?: { type: string } } =>
        p.type === "reasoning" && i >= startIndex && i <= endIndex
    )

    if (reasoningParts.length === 0) return false

    return reasoningParts.every((p) => p.status?.type === "complete")
  })

  const [userOpen, setUserOpen] = useState<boolean | null>(null)

  const isControlledByUser = userOpen !== null
  ...
  const isOpen =
    isReasoningStreaming || !isReasoningComplete
      ? true
      : isControlledByUser
        ? userOpen
        : false
