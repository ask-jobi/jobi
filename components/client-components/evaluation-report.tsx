...
import { AlertCircle, Target, AlertTriangle, Loader2, Sparkles, CheckCircle2, XCircle } from 'lucide-react'
...
export function EvaluationReport({ evaluation }: EvaluationReportProps) {
  ...
  const getSeverityIcon = (severity: "critical" | "important" | "minor") => {
    switch (severity) {
      case 'critical':
        return <AlertTriangle className="w-4 h-4 text-red-600 mt-0.5 shrink-0" />
      case 'important':
        return <AlertCircle className="w-4 h-4 text-orange-600 mt-0.5 shrink-0" />
      case 'minor':
        return <AlertCircle className="w-4 h-4 text-yellow-600 mt-0.5 shrink-0" />
      default:
      ...
      <div className="flex items-center gap-2">
        {
          !loading && (
            <>
              <Button onClick={refreshEvaluation} variant="outline">{t("refreshEvaluation")}</Button>
              <Button 
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex items-center justify-between py-2 border-b border-border last:border-b-0">
                    <span className="text-sm font-medium">{t("gates.atsScreening")}</span>
                    <div className="flex items-center gap-2">
                      {getGateStatus(evaluation.gates.ats).icon}
                      <span className={`text-sm font-medium ${getGateStatus(evaluation.gates.ats).color}`}>
                  ...
                  <div className="flex items-center justify-between py-2 border-b border-border last:border-b-0">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-sm font-medium ${getGateStatus(evaluation.gates.hr).color}`}>
                  ...
                  {/* Hiring Manager Fit */}
                  <div className="flex items-center justify-between py-2 border-b border-border last:border-b-0">
                    <span className="text-sm font-medium">{t("gates.hiringManagerFit")}</span>
                    <div className="flex items-center gap-2">
                      {getGateStatus(evaluation.gates.hiringManager).icon}
                      <span className={`text-sm font-medium ${getGateStatus(evaluation.gates.hiringManager).color}`}>
                    ...
                    <div key={index} className="p-3 bg-muted rounded-md space-y-2">
                      ...
                      {gap.evidence && (
                        <div className="mt-2 pt-2 border-t border-border">
                          <p className="text-xs font-medium text-muted-foreground mb-1">{t("evidence")}:</p>
