...
import {
  appendPatchHistoryAtom,
  undoPatchAtom
} from "@/lib/store/suggestion-patch-history"
...
function SuggestionPatch({
  ...
  const handleApplyPatch = () => {
    ...
    if (block) {
      ...
      setValue(
        `${section.section}.blocks.${section.blockIndex}.content` as any,
        section.optimizedContent ?? "",
        {
          shouldDirty: true,
          shouldTouch: true,
  ...
  return (
    <div>
      <div>
        <section className="mb-2">{section.reason}</section>
        <section>
          {words.map((it, index) => {
            if (it.added) {
              return (
