...
上传 PDF 创建 `Application Resume` 的主链路已经从 route 内的大编排收口为独立 intake module。当前实现把 request 校验、SSE 适配、领域编排、rollback 与 persist 分开，并以稳定的 SSE 事件协议驱动前端上传状态机。
...
- commit 前取消会执行 rollback，并以 `intake.cancelled` 结束。
...
- `server/intake/orchestrator.test.ts`：成功链路、step 失败、commit 前取消、commit 后语义、terminal event 唯一性。
