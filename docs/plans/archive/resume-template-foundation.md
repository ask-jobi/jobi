...
> ⚠️ **已过期（superseded）** — 后续 `default` / `modern` 模板已通过 `section-entries.tsx` 共享渲染层、`sectionOrder` 统一排序语义等方式逐步演进。`default` 和 `modern` 现已对齐（参考 `2026-05-20-resume-section-manual-reorder.md`）。本文件的模板底座方案不再反映当前实现，不应作为参考。
