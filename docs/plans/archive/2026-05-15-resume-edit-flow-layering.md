...
> ⚠️ **已过期（superseded）** — 本计划基于 `draft + autosave + 页面级 RHF` 旧模型编写，该模型已在 `2026-05-18-remove-draft-resume-state.md` 中被整体移除。当前编辑页使用 `persisted resume only` 模型，本文件的架构描述不再反映实际情况，不应该作为后续实现参考。
